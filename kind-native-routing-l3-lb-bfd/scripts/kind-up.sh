...
#   - Inter-node PodCIDR traffic (learned from Cilium BGP CP)
#   - Client-net return path (learned from FRR1 via BGP)
#   - Internet-bound traffic (forwarded to Docker bridge gateway 172.19.0.1)
# This eliminates the need for auto-direct-node-routes, per-node PodCIDR
# static routes, and explicit client-net return routes.
